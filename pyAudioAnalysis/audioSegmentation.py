...
def mid_term_file_classification_chanting(input_file, model_name, model_type,
                                 plot_results=False, gt_file=""):
    """
    Copy from mid_term_file_classification, for Chanting classisication use.
    This function performs mid-term classification of an audio stream.
    Towards this end, supervised knowledge is used,
    i.e. a pre-trained classifier.
    ARGUMENTS:
    ...
    # Load grount-truth:
    labels_gt, class_names_gt, accuracy, cm = \
        load_ground_truth(gt_file, newlabels, class_names, mid_step, plot_results)
    # plot_segmentation_results = (flagsInd,flagsInd,classesAll,2)
    return newlabels, class_names, accuracy, cm, segs, classes
...
def smooth_bumps(segs, classes, classesAll, mid_step, n_mid_step, target_class):
    new_segs = []
    new_classes = []
    new_segs_id = 0
    for i, (seg, cls) in enumerate(zip(segs, classes)):
        try:
            if classesAll[int(classes[i-1])] == classesAll[int(classes[i+1])] and seg[1] - seg[0] <= mid_step*n_mid_step and \
                    classesAll[int(classes[i-1])]== target_class:
                        
                new_segs[-1][1] = seg[1]
            else:
                new_segs.append([seg[0], seg[1]])
    ...
    return new_segs, new_classes

def combine_class(segs, classes, classesAll):
    #Combine successive class(Tyler)
    new_segs_filter = []
    new_classes_filter = []
    new_segs_filter_id = 0
    # print(new_classes)
    for j, (seg, cls) in enumerate(zip(segs, classes)):
        try:
            if classesAll[int(classes[j-1])] == classesAll[int(classes[j])] or seg[1]-seg[0]<=2:
                ...
                new_segs_filter.append([seg[0], seg[1]])
                new_classes_filter.append(cls)
                new_segs_filter_id += 1
    ...
    return new_segs_filter, new_classes_filter
